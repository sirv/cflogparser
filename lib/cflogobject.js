    ...
    csv()
        ...
        .transform(function(row, index){
            ...
            _.each(['cs(User-Agent)', 'cs(Referer)', 'cs-uri-stem', 'cs-uri-query', 'cs(Cookie)'], function(field){
                ...
                } catch (e) { // try to fix some 'Malformed URI' exceptions with strange UTF errors
                    console.log('WARNING: URI parse error: ', row[field], ', trying with unescape() ...')
                    row[field] = decodeURIComponent(unescape(row[field]));
